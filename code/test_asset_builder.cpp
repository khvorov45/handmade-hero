...
LoadGlyphBitmap(loaded_font* Font, uint32 Codepoint, hha_asset* Asset) {
    ...
    // NOTE(sen) Horizontal advance with this character's filled region,
    // post-region and next character's pre-region
    ...
    for (uint32 OtherGlyphIndex = 0; OtherGlyphIndex < Font->MaxGlyphCount; ++OtherGlyphIndex) {
        real32 OtherPre = 0.0f;
        if (OtherGlyphIndex != 0 && GlyphIndex != 0) {
            ABC OtherABC;
            GetCharABCWidthsW(GlobalFontDeviceContext, Codepoint, Codepoint, &OtherABC);
            OtherPre = (real32)(OtherABC.abcA);
        }
        Font->HorizontalAdvance[GlyphIndex * Font->MaxGlyphCount + OtherGlyphIndex] = ThisFilledAndPost + OtherPre;
    }
